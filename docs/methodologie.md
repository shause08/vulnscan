...
Binaire ELF
    ...
    │        └─ ASan                  (build instrumenté, rapport d'erreur riche)
    │
...
Le catalogue comprend 17 fonctions dangereuses réparties en catégories :

- **Dépassement de tampon sans borne** : `gets` (CRITICAL), `strcpy`, `strcat`, `sprintf` (HIGH)
- **Chaîne de format** : `printf`, `fprintf`, `syslog` (HIGH)
- **Dépassement contrôlable** : `read`, `recv`, `memcpy`, `memmove` (MEDIUM — dépend des arguments)
- **Exécution de commande** : `system`, `popen` (CRITICAL), `execve` (HIGH)
...
**Puits** : `strcpy`, `strcat`, `sprintf`, `printf`, `fprintf`, `system`, `memcpy`, etc.
...
vulnscan cherche un binaire `*_asan` juxtaposé au binaire `*_vuln` analysé. Si trouvé, il le lance avec les entrées crashantes découvertes par le fuzzer et l'option :
...
Le parseur gère les cas particuliers :
- **DEADLYSIGNAL** (nested bug) : gets() peut déborder dans la shadow memory d'ASan, causant un crash récursif. vulnscan re-scanne le rapport pour l'erreur initiale.
- **`__interceptor_`** : les frames ASan internes sont ignorées pour identifier la première frame utilisateur.

## Phase 5 — Corrélation et sévérité

### Corrélation statique + dynamique

Lorsque l'analyse statique et l'analyse dynamique détectent la même classe de vulnérabilité, la confiance est élevée à `"both"`. Cela réduit les faux positifs et identifie les findings les plus fiables.
