"""Phase 6 — report generation: HTML."""
...
class TestReportFromRealScan:
    ...
    def test_all_corpus_binaries_produce_valid_html(self):
        from vulnscan.pipeline import scan
        from vulnscan.report.generator import render_html
        ...
        for name in binaries:
            ...
            html = render_html(result)
            assert "<!DOCTYPE" in html, f"{name}: HTML missing DOCTYPE"
            assert name in html, f"{name}: binary name not in HTML"
